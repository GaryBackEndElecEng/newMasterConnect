...
apiProtectAdminHome.interceptors.response.use(
    ...
    async (error)=>{
        ...
        ///detail/:id
        if(error.response.status===401 && originalRequest.url ===`${apiProtectAdminHome.baseURL}/token/refresh`){
            window.location.href = '/signin';
            return Promise.reject(error);
        }
        if(error.response.statusText === 'unauthorized' && originalRequest.url ===`${apiProtectAdminHome.baseURL}/signin`){
            window.location.href = `/signin`;
            return Promise.reject(error);
        }
        if(error.response.data.code ==='token_not_valid' && error.response.status === 401 && error.response.statusText === 'unauthorized'){
            ...
            if(refresh_token){
                ...
                if(tokenParts.exp > now){
                    return apiProtectAdminHome.post('/token/refresh/',{refresh:refresh_token}).then((response)=>{
                        localStorage.setItem("access_token",response.data.access_token);
                        localStorage.setItem("refresh_token",response.data.refresh_token);
                        apiProtectAdminHome.Instance.defaults.headers['Authorization']= `Bearer ${response.data.access}`
                        return apiProtectAdminHome(originalRequest);
                    })
                    .catch(
                        (err)=>{console.log(err)}
                    );
...
export default apiProtectAdminHome;