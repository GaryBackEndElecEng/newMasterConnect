    ...
    useEffect(()=>{
      ...
      if(generalInfo.loaded){
        ...
        generalInfo.data.siteArray.forEach((ele,index)=>{
          ...
          if(site && site.split("").length>0 ) return arr.push({id:index,site:site,icon:<WebIcon sx={{ml:1,mr:1,color:"blue"}}/>});
          let fb=ele.split("fb:")[1];
          if(fb && fb.split("").length>0 ) return arr.push({id:index,media:fb,icon:<FacebookIcon sx={{ml:1,mr:1,color:"blue"}}/>});
          let twitter=ele.split("twitter:")[1];
          if(twitter && twitter.split("").length>0) return arr.push({id:index,media:twitter,icon:<TwitterIcon sx={{ml:1,mr:1,color:"blue"}}/>});
          let linkedln=ele.split("linkedln:")[1];
          if(linkedln && linkedln.split("").length>0 ) return arr.push({id:index,media:linkedln,icon:<LinkedInIcon sx={{ml:1,mr:1,color:"blue"}}/>});
        });
        setSiteArr(arr);
          ...
          >
            ...
            <Grid container spacing={{xs:0,sm:1}} sx={{padding:"0.5rem",justifyContent:"flex-start"}}>
              {siteArr && siteArr.map((obj,index)=>(
                <Grid item xs={12} sm={6} key={`${obj.id}-particular-${index}`} sx={{borderTop:"1px solid white"}}>
                  {obj.site &&<Typography component="h1" variant="body1">{ obj.site}-{ obj.icon}</Typography>}
                  {obj.media && <Typography component="h1" variant="body1">{obj.media}-{obj.icon}</Typography>}
                  </Grid>
              ))}
            </Grid>
