...
useEffect(()=>{
 ...
 if(userQuestionArray.data && storeUUID.loaded){
  userQuestionArray.data.filter(obj=>(JSON.parse(obj.yesno)===false)).forEach((obj,index)=>{
    storeUUID.data.additionalCharArr.forEach((obj2,index)=>{
      if(obj2.includes("hits") && obj.Q.includes("hits")){
        arr.push({"Q":obj.Q,"ans":obj2})
      }
      if(obj2.includes("type of database") && obj.Q.includes("type of database")){
        arr.push({"Q":obj.Q,"ans":obj2})
      }
      if(obj2.includes("industry") && obj.Q.includes("industry")){
        arr.push({"Q":obj.Q,"ans":obj2})
      }
      if(obj2.includes("existing site") && obj.Q.includes("existing site")){
        arr.push({"Q":obj.Q,"ans":obj2})
      }
      if(obj2.includes("company") && obj.Q.includes("company")){
        arr.push({"Q":obj.Q,"ans":obj2})
      }
    });
  });
