...
const Description = ({ obj, title }) => {
    ...
    const checkLength = obj.desc !== "" ? true : false;
                    ...
                    >
                        {
                            (activate.loaded && activate.id === obj.id) ?
                                <CustEffect1> {obj.desc}</CustEffect1>

                                :
                                <CustEffect style={{ color: theme.palette.common.blueGreyDark, }}> {obj.desc.slice(0, 100)}... </CustEffect>
